...
\address{IN$d$AM - Marie Curie Actions fellow, Universit\`a ``La Sapienza'', Roma, Italy.}
...
\section{$\fsl_2$-Tilings of the Plane}
  ...
  In fact, it is shown in \cite{AssemReutenauerSmith} that any ``frontier'', or broken line of $1$'s in the plane, can be completed into a unique $\fsl_2$-frieze.
  \saySS{We never introduced $\fsl_2$-friezes; should we replace this?}
  ...
  The following $\fsl_2$-tiling will be relevant in our higher dimensional analisys.
...
\section{$\fsl_2$-Tilings in Higher Dimensions}
  ...
  We call \emph{signature matrix} a skew-symmetric $n\times n$ matrix $\bepsilon=(\epsilon_{k\ell})$ with $\epsilon_{k\ell}=\pm 1$ whenever $k\neq \ell$. 
  \saySS{I decided to switch to matrix notation because it makes the argument for higher rank slightly easier to write; I am open to reverting to the previous notation if you prefer. Note that the notation is redundant: the bottom part of $\bepsilon$ does not play any role. We could probably add a comment on this.}
  \begin{definition}
    Fix a signature matrix $\bepsilon$.  
    An array $(a_{\bi})_{\bi\in\ZZ^n}$ with $a_{\bi}\in\ZZ_{>0}$ is called an \emph{$\bepsilon-\fsl_2$-tiling of $\ZZ^n$} if for each $1\le k<\ell\le n$ we have
    \saySS{Can we make this into a symmetric definition to allow also $k>\ell$?}
    \begin{equation}\label{eq:higher sl2 recursion}
      a_{\bi+\be_\ell}a_{\bi+e_k}-a_{\bi}a_{\bi+\be_k+\be_\ell}=\epsilon_{k\ell}.
    \end{equation}
  \end{definition}
  ...
  \begin{definition}
    Let $\epsilon = 1$ (resp. $\epsilon=-1$) and $\bepsilon$ be such that $\epsilon_{k\ell}=\epsilon$ whenever $k<\ell$. 
  ...
  \begin{proof}
    ...
    Therefore al the $\epsilon_j$ coincide and either $\bepsilon$ or $\bepsilon^{(n)}$ yields the unique anti-$\fsl_2$-tiling of $\ZZ^n$.
