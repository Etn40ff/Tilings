...
\section{$\fsl_2$-Tilings in Higher Dimensions}
  ...
  \begin{lemma}\label{le:flip}
    Let $\bepsilon=(\epsilon_{k\ell})$ be any signature matrix and write $\bepsilon^{(r)}=(\epsilon'_{k\ell})$ where $\epsilon'_{k\ell}=-\epsilon_{k\ell}$ if $k=r$ or $\ell=r$ (and $k\neq\ell$)
    \sayPT{One probably needs $k\ne\ell$ here} 
    \saySS{Yes, this was a leftover of the old notation where $\bepsilon$ was skew-symmetric. It is fixd and both comments can be removed now.}
    and $\epsilon'_{k\ell}=\epsilon_{k\ell}$ otherwise. 
  ...
  \begin{definition}
    ...
    \saySS{Same here; now it is fixed}
  ...
  \begin{theorem}
    ...
    \saySS{Yes, that's the one I meant. How do we fix this?}
  \end{theorem}  
  \begin{proof}
    ...
    \sayPT{Shouldn't this be $\{a_{\bi+\be_k},a_{\bi-\be_\ell}\}=\{1,2\}$, or it is essentially the same?}
    \saySS{I think they are the same: because of lemma 8 $a_{\bi-\be_\ell}$ should be the same as $a_{\bi-\be_k}$; am I right?}
