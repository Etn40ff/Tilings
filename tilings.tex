...
\section{$\fsl_2$-Tilings of the Plane}
  ...
  \sayPGP{My bad; it should be $\fsl_2$-tiling.  I changed it.}
...
\section{$\fsl_2$-Tilings in Higher Dimensions}
  ...
  A \emph{signature matrix} is any skew-symmetric $n\times n$ matrix $\bepsilon=(\epsilon_{k\ell})$ with $\epsilon_{k\ell}=\pm 1$ whenever $k\neq \ell$. 
  ...
  \sayDR{Why is the matrix skew-symmetric?  It seems to me that we don't actually care about the ordering of the indices and this matrix should be symmetric.}
  \sayPGP{I agree with Dylan.  Making the matrix symmetric would allow us to drop the $k<\ell$ condition in Definition 5, thus addressing Salvatore's comment below. The matrix notation is very good, though.}
  \begin{definition}
  ...
  \begin{theorem}
    ...
    \sayPGP{Should we make explicit in the statement that all entries are Fibonacci numbers?}
  \end{theorem}  
  \begin{proof}
